...
class TestServiceRTC(object):
    ...
    def driver(self):
        ...
        try:
            if isinstance(driver, dict):
                ...
                header = {"Session-Token": driver["token"], "Peer-Id": driver["peer_id"], "x-forwarded-for":driver["x-forwarded-for"]}
    ...
    def test_admin_agent_msg_closed(self, driver):
        ...
        data = {"node_id": driver["node_id"], "data": {"type": "closed", "session_id": driver["token"], "reason": "test", "code": 1}}
        response = requests.post(url, data=json.dumps(data), headers=header)
        log.debug("response status code is {0}, text is {1} ".format(response.status_code, response.text))
        assert response.status_code == 200
        check_response = json.loads(response.text)
